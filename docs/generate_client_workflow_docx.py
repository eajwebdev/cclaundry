...
sections = [
    ...
    ("h1", "Diagram Overview"),
    ("p", "The top row shows the normal status flow: Booking, Pickup Confirmed, Collected, POS Job Order, Washing, Drying and Finishing, Ready, and Released. Cancellation is shown separately as an exception. The three columns below show each user's actions."),
    ("image", ""),
    ...
    ("bullet", "Customer can track the booking, rider location during active trips, and laundry progress through washing, drying, folding or steaming, and ready status."),
    ...
    ("bullet", "Cashier opens the POS tag button, searches by tag, booking, or customer, or chooses one of the nine oldest waiting bags."),
    ("bullet", "Staff verifies the bag, weight, services, final price, and payment, then creates the job order in the same POS used for walk-ins."),
    ("bullet", "Laundry is processed through washing, drying, and folding or steaming as needed."),
    ...
    ("bullet", "Rider collects laundry and records its bag tag and payment details. A tag used that day or still on an active bag cannot be reused."),
    ...
    ("p", "Next is the Admin or Staff side. Staff reviews and confirms the booking and assigns a rider. When the tagged bag arrives, the cashier opens the POS tag button to find the booking and load its services. The cashier checks weight, final price, and payment, creates the job order, then moves it through washing, drying, and folding or steaming."),
    ("h2", "Script: Explain Driver / Rider"),
    ("p", "On the Driver or Rider side, the rider sees available or assigned pickup runs. They accept the run, go to the customer, attach and record a bag tag, record payment details if needed, and bring the laundry to the branch. The tag cannot be reused that day or while another active bag has it. If the customer selected delivery, the rider later delivers the finished laundry back."),
    ("h2", "Script: Explain Status Row"),
    ("p", "The status row at the top shows the normal life cycle of one booking: booked, confirmed, collected, loaded into a POS job order, washing, drying and finishing, ready, then released. The customer can see the production stages. Cancellation is an exception shown below the main flow."),
    ("h2", "Script: Explain Handoffs"),
    ("p", "The handoff is the key. Customer to Admin is the online booking. Admin to Rider is the assigned pickup. Rider to Cashier is the tagged bag returned to the branch and loaded in POS. Staff or Rider back to Customer is the final counter release or delivery."),
    ("h2", "Script: Closing"),
